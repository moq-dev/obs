...
static void hang_source_tick(void *data, float seconds);
static void hang_source_render(void *data, gs_effect_t *effect);
...
static void hang_source_render(void *data, gs_effect_t *effect)
{
	...
	obs_source_draw(ctx->source, ctx->frame_buffer, FRAME_WIDTH * 4, FRAME_HEIGHT, false);
}
...
static void on_catalog(void *user_data, int32_t catalog)
{
	...
	int32_t track = moq_consume_video_track(ctx->consume, 0, 0, on_video_frame, ctx);
	if (track < 0) {
...
static void hang_source_reconnect(struct hang_source *ctx)
{
	...
	int32_t catalog = moq_consume_catalog(ctx->consume, on_catalog, ctx);
	if (catalog < 0) {
		LOG_ERROR("Failed to subscribe to catalog: %d", catalog);
		return;
	}

	LOG_INFO("Connecting to MoQ broadcast: %s @ %s", ctx->broadcast, ctx->url);
}
...
void register_hang_source()
{
	...
	info.video_tick = hang_source_tick;
	info.video_render = hang_source_render;
